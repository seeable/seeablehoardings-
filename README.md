...
- **Status:** **Roadmap complete (Phases 0–13)** + full SEEABLE brand identity system. **Brand:** black/charcoal/gold dark theme end-to-end (`app/globals.css` — semantic token *names* unchanged from the prior light system, only values redefined, so the whole component tree re-themed automatically); official portal-doorway logo (`public/logo-square.png`, `logo-horizontal.png`) in every nav surface, auth screens, landing, and error pages; tagline "SEE IT. STEP INTO IT." on the split-layout auth gateway and landing hero; gold reserved for primary CTAs, selected/active states, and focus rings per brand guidelines (never decorative). Zero WCAG AA violations (axe-core, scanned across auth/landing/discover/publisher/admin) — caught and fixed a real contrast gap in the muted-text token during the rebuild. **Phase 13:** Deployment, launch prep & operations — production env setup (GitHub Actions + Cloudflare Workers), staging-gated migration pipeline, `pg_cron` monitoring (staleness alert), backup/restore procedures, launch-inventory gate (50+ approved listings), production limits & upgrade triggers, and on-call runbooks. **Phase 12:** Testing & QA — deterministic seed (~60 listings across all hoarding types, 2+ Publishers, requests in all states), 6 critical-flow E2E tests (Playwright: Viewer discovery→booking, concurrent conflicts, Publisher onboarding, rejection workflow, SLA expiry, suspended-Publisher isolation). Framework ready for rule-ID integration coverage (`npm run test:e2e` in CI). **Phase 11:** Security hardening — CSRF checks, CSP/HSTS/Permissions-Policy, `/terms`/`privacy` pages, content-moderation + dispute runbooks. **Phase 10:** Analytics (7 KPIs, in-transaction notifications). **Phase 9:** Admin platform (5 features). **Billboard import:** 62 of 68 real site-survey photos live in Discover; 5 DRAFT by design, 1 excluded. All systems green; ready for production launch.
